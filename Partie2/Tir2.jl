...
"""
Methode de tir Multiple avec comme inconnus p0 , t1 , t2 sont respectivement 
l'adjoint initial , les temps d'entrée et de sortie de l'axe singulier.
"""
...
#Ici p1 , p2 représentent les 2 composantes de p à l'entrée de l'arc singulier
function commut(m1,m2,p1,p2)
    psi = 1/(1+m1/si1 + m2/si2)
    return psi + 0.5*p1*alpha1*psi + 0.5*p1*a1*m1 + 0.5*p2*alpha2*psi + 0.5*p2*a2*m2
end
...
function shoot(k1,k2,t1,t2)
    ...
    return [p3[1], p3[2],commut(m1[1],m1[2],p1[1],p1[2]),0.0]
end
# itéré initiale pour la méthode indirecte de tir (Mettre de bonnes valeurs ici)
# y1 est [p01,p02,t1,t2]
